...
import { findExistingAsset, findFrameForNode, readFrameFile, readIndex, readManifest } from "./cache";
import { formatCharOverrides, formatPaints, formatTextStyle } from "./paints";
import type { Breakpoint, FigmaNode } from "./types";

/** Depth-first search for a node by id within a subtree. */
 ...
 */
export function renderDigest(target: FigmaNode, maxDepth: number, leavesOnly: boolean): string {
	...
	const describe = (n: FigmaNode, depth: number) => {
		...
		if (stroke) bits.push(`stroke=[${stroke}] w=${n.strokeWeight ?? "?"} align=${n.strokeAlign ?? "?"}`);
		if (n.layoutMode && n.layoutMode !== "NONE") {
			const padding = [n.paddingTop, n.paddingRight, n.paddingBottom, n.paddingLeft]
				.map((v) => v ?? 0)
				.join("/");
			bits.push(
		...
		lines.push(`${pad}- [${n.type}] "${n.name}" ${geo}${bits.length ? ` · ${bits.join(" · ")}` : ""}`);
	...
	lines.push(`# ${target.name} [${target.type}] ${width}×${height} — ${countNodes(target)} nodes total`);
	const render = findExistingAsset(target.id);
	if (render) lines.push(`# render: .design/figma-cache/${render}  (visual reference — diff against this)`);
	lines.push("# geometry is parent-relative to this node's top-left (0,0). All fields, no filtering.");
	lines.push("");
	walk(target, 0);
	return lines.join("\n");
}
...
export function renderImageInventory(target: FigmaNode): string {
	...
	const visit = (n: FigmaNode) => {
		const fill = (n.fills ?? []).find((f) => f.type === "IMAGE" && f.visible !== false);
		if (fill) {
			const bb = n.absoluteBoundingBox;
			const geo =
				bb && origin
			...
			slots.push(`- ${n.id}  "${n.name}"  ${geo}  ${fill.scaleMode ?? "?"}  ${tail}`);
		}
...
function resolveQuery(query: string, bp: string | undefined): Resolution {
	...
	if (!entry) return { ok: false, message: `Unknown index name "${query}" — run \`figma list\` to see targets.` };

	const variants = entry.node;
	if (bp) {
		const id = variants[bp as Breakpoint];
		if (!id) return { ok: false, message: `Target "${query}" has no "${bp}" variant.` };
		return { ok: true, id };
	}
	if (variants.desktop) return { ok: true, id: variants.desktop };
	const keys = Object.keys(variants) as Breakpoint[];
	if (keys.length === 1) {
		const id = variants[keys[0]];
		if (id) return { ok: true, id };
	}
	return {
		ok: false,
		message: `Target "${query}" is responsive (${keys.join(", ")}) — specify --bp=desktop|tablet|mobile.`,
	};
}
...
export async function runRead(args: ParsedArgs): Promise<number> {
	...
	if (!query) {
		console.error("Usage: figma read <nodeId|name> [--depth=N] [--leaves] [--bp=desktop|tablet|mobile] [--raw]");
		return 1;
	}
	const raw = args.flags.has("raw");
	const leavesOnly = args.flags.has("leaves");
	const depthFlag = args.flags.get("depth");
	const maxDepth = typeof depthFlag === "string" ? Number(depthFlag) : Number.POSITIVE_INFINITY;
	const bpFlag = args.flags.get("bp");
	const bp = typeof bpFlag === "string" ? bpFlag : undefined;
	...
	if (!frameId) {
		console.error(`Node ${nodeId} is not in the cache — run \`figma collect\` first.`);
		return 1;
	}
	...
	console.log(raw ? JSON.stringify(target, null, 2) : renderDigest(target, maxDepth, leavesOnly));
	return 0;
}
