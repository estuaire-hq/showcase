...
- [ ] T001 Vérifier un arbre de travail git propre sur la branche `002-nextjs-16-migration` (`git status`) et noter le SHA de baseline v15 comme point de comparaison
- [ ] T002 [P] Lancer `npm run lint` (Biome, script `package.json` → `lint`) sur la baseline v15 et confirmer le vert
- [ ] T003 [P] Lancer `npm run build` sur la baseline v15, confirmer le vert, vérifier que `.next/standalone/` est produit et noter le temps de build (référence)
- [ ] T004 [P] Capturer les captures de référence des pages clés (accueil + scroll complet) pour le diff de parité final (Principe VII, SC-001) — stocker dans un dossier de travail (hors `public/`)
...
- [ ] T005 Lancer le codemod officiel `npx @next/codemod@canary upgrade latest` à la racine du dépôt et **relire chaque diff** avant de stager (research §D2 ; le codemod gère config `turbopack`, `middleware → proxy`, retrait `unstable_`/`experimental_ppr`)
- [ ] T006 Épingler/confirmer les deps cœur dans `package.json` : `next@16.2.9`, `react@^19.2.7`, `react-dom@^19.2.7` ; ajouter en devDeps `@types/react@^19.2.17`, `@types/react-dom@^19.2.3` ; lancer `npm install` pour recalculer `package-lock.json` (FR-001, research §D1)
- [ ] T007 [P] Confirmer Node ≥ 20.9 dans `Dockerfile`, `.github/workflows/ci.yml`, `.github/workflows/seed-sanity.yml` (FR-015 — `node:22` attendu partout, aucun changement requis)
...
- [ ] T008 [P] [US1] Mettre à jour le script `dev` de `package.json` : `next dev --turbopack` → `next dev` (retrait du `--turbopack` redondant) ; confirmer que `build`/`start`/`lint`/`typegen`/`seed*` restent intacts (FR-007)
- [ ] T009 [P] [US1] Compléter/vérifier la migration `middleware → proxy` : `src/middleware.ts` renommé en `src/proxy.ts`, fonction exportée `middleware` → `proxy`, retrait de `runtime: "nodejs"` du `config`, en-tête de commentaire réécrit (runtime `proxy`) ; matcher + logique du gate **strictement préservés** (FR-003, research §D3, C2)
- [ ] T010 [P] [US1] Mettre `revalidateTag("sanity")` en conformité v16 à deux arguments → `revalidateTag("sanity", "max")` dans `src/app/api/revalidate/route.ts` (FR-004, research §D4)
- [ ] T011 [P] [US1] Revoir/confirmer `next.config.ts` pour les options v16 (pas de `images.domains`, `experimental.ppr`, `serverRuntimeConfig`/`publicRuntimeConfig`, `amp`, option `eslint` ; `experimental.turbopack` → `turbopack` racine si présent) ; confirmer `output: "standalone"` + `images.remotePatterns` intacts (FR-008, research §D6)
- [ ] T012 [P] [US1] Scanner `src/` pour les API retirées en v16 (`next/amp`, `useAmp`, `serverRuntimeConfig`/`publicRuntimeConfig`/`next/config`, `next/legacy/image`, `unstable_rootParams`, `next lint`) et confirmer **zéro occurrence** (FR-017, SC-010)
- [ ] T013 [P] [US1] Confirmer l'usage exclusif des Async Request APIs : vérifier `await draftMode()` dans `src/app/layout.tsx` et l'absence de tout accès synchrone résiduel à `cookies`/`headers`/`params`/`searchParams` (FR-005)
- [ ] T014 [US1] Lancer `npm run build` (Turbopack, défaut v16) sous Next 16 ; confirmer le vert + `.next/standalone/` produit ; en cas d'échec dû à une config webpack implicite, documenter et appliquer le repli `next build --webpack` (FR-006, SC-007, C7)
- [ ] T015 [US1] Smoke du gate « coming soon » dans ses **4 états** via `npm run dev` (token absent → ouvert ; token défini sans cookie → placeholder ; `/v/<token>` valide → pose cookie + redirige `/` ; cookie valide → site réel ; mauvais token → placeholder) (FR-003, SC-002, C2)
- [ ] T016 [US1] Vérifier le rendu des images `cdn.sanity.io` sous les nouveaux défauts `next/image` v16 (qualité, cache, `imageSizes`) ; n'ajuster `images.qualities`/`imageSizes`/`minimumCacheTTL` dans `next.config.ts` **que si** dégradation perçue (FR-009, research §D7)
- [ ] T017 [US1] Vérifier que `src/app/sitemap.ts` (statique, pas de `generateSitemaps`) et `src/app/robots.ts` (keyé sur `SITE_PREVIEW_TOKEN`) génèrent la même sortie qu'en v15 (FR-013, C6)
- [ ] T018 [US1] Vérifier le scroll fluide Lenis + reveals GSAP (`@gsap/react`) y compris sur les transitions inter-pages, respect de `prefers-reduced-motion`, et que le `<Script>` Umami charge toujours dans `src/app/layout.tsx` (FR-014, SC-009, Principe VI, C8)
- [ ] T019 [US1] Parcourir toutes les pages `(site)` sur le build Next 16 et confirmer la parité pixel/comportement vs la référence v15 (T004), **zéro erreur console/hydratation**, et que `/studio` (encore v4) se monte (SC-001, SC-008, C8)
...
- [ ] T020 [US2] Monter les deps Sanity dans `package.json` : `sanity@^5.31.1`, `@sanity/vision@^5.31.1`, `next-sanity@^13.0.12`, `@sanity/image-url@^2.1.1`, `styled-components@^6.1.15` ; `npm install` (`@sanity/client@7` arrive en transitif ; `@sanity/locale-fr-fr` inchangé) — ⚠️ version `next-sanity` résolue = **@13** (research §D1), pas @12 comme rédigé dans spec.md (FR-002)
- [ ] T021 [US2] Régénérer les types : `npm run typegen` (extract schéma + generate) → `src/sanity.types.ts` ; confirmer qu'aucun type de contenu n'est tapé à la main (FR-012, Principe IX, C5)
- [ ] T022 [US2] Vérifier la stratégie cache/live sous `@sanity/client@7` dans `src/lib/sanity/client.ts` + `src/lib/sanity/live.ts` : publié via CDN (`useCdn:true`, sans token) + brouillons via `serverToken` dans `defineLive` ; confirmer `apiVersion` (`2026-03-01`) + `stega` (FR-011, research §D8, C3)
- [ ] T023 [US2] Vérifier que `urlFor(...)` dans `src/lib/sanity/image.ts` sous `@sanity/image-url@2` génère les mêmes URLs `cdn.sanity.io` (taille/format/qualité) qu'en v1 ; repli en v1 si la sortie change de façon indésirable (research §D8)
- [ ] T024 [US2] Lancer `npm run build` et confirmer le typecheck **vert** avec `src/sanity.types.ts` régénéré (FR-012, SC-004, C5)
- [ ] T025 [US2] Lancer `npm run seed:check` (dry-run, offline, sans token) et confirmer le vert — champs `required` + assets référencés présents sous TypeGen v5 (FR-012, C5)
- [ ] T026 [US2] Smoke `/studio` **v5** via `npm run dev` : montage du Studio, auth, navigation desk (structure custom), édition, plugin Vision, locale `fr-FR` ; `export const dynamic = "force-static"` de la route conservé (FR-010, SC-003, C4)
- [ ] T027 [US2] Vérifier draft mode + `<SanityLive />` + `<VisualEditing />` (aperçu en direct) dans `src/app/layout.tsx` ; contenu publié servi via CDN (FR-011, SC-005, C3)
- [ ] T028 [US2] Revalidation de bout en bout : publier un document → `POST /api/revalidate` (signature valide) → confirmer l'invalidation effective des caches et l'apparition de la valeur publiée côté site (contrat 200 + revalidation effective, pas seulement la compilation) (FR-004, SC-005, C1)
...
- [ ] T029 [US3] Lancer `npm run dev` et confirmer que le serveur démarre avec Turbopack (défaut v16) et que le hot-reload fonctionne (FR-007)
- [ ] T030 [P] [US3] Lancer `npm run lint` (Biome) sur la branche migrée intégrée et confirmer le vert (FR-016, SC-006)
- [ ] T031 [P] [US3] Lancer `npm run build` sur la branche migrée intégrée et confirmer le build de production vert + sortie `standalone` (SC-006, SC-007)
- [ ] T032 [US3] Pousser la branche `002-nextjs-16-migration` et confirmer que les jobs CI `lint` + `build` (`.github/workflows/ci.yml`) passent au vert (FR-016, SC-006)
- [ ] T033 [US3] Confirmer le chemin seed CI (`npm run seed:ci` / `seed:check`, `.github/workflows/seed-sanity.yml`) fonctionnel sous TypeGen v5 (FR-012)
...
- [ ] T034 [P] Rédiger l'ADR `docs/vault/decisions/0008-nextjs-16-sanity-5-migration.md` : cibles de version (Next 16 + Sanity v5), exclusion Sanity v6, périmètre *lift*, choix Turbopack, pivot React 19.2 (FR-018)
- [ ] T035 [P] Mettre à jour l'ADR `docs/vault/decisions/0007-*.md` (coming-soon gate) : remplacer les références `middleware` par `proxy`
- [ ] T036 Construire l'image Docker `standalone` en local (`docker build -t estuaire:next16 .`) et la lancer (`docker run --rm -p 3000:3000 estuaire:next16`) ; confirmer le démarrage et le service sur `:3000` (FR-006, SC-007, C7)
- [ ] T037 Dérouler l'intégralité des contrats de régression `specs/002-nextjs-16-migration/contracts/regression-contracts.md` (C1→C8) et confirmer chaque invariant (gate final)
- [ ] T038 Déploiement : merge sur `main` → build & deploy Coolify → smoke prod (gate, contenu, animations) ; confirmer que le build Coolify tourne en Node ≥ 20.9 (SC-001…SC-009)
- [ ] T039 [P] Suivi de gouvernance (companion, hors exécution stricte de ce plan) : amender la **constitution** (table « Stack applicative » : Next 15→16, Sanity v4→v5, lever `TODO(ANIMATION_LIB)` = GSAP) avec Sync Impact Report + bump de version (plan §Suivis de gouvernance)
