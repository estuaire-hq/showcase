...
- [X] T031 [P] **Gouvernance** : rédiger `docs/vault/decisions/00XX-email-sending-ovh-emailpro.md` (envoi = OVH Email Pro SMTP ; réception = boîtes partagées Exchange ; routage CMS ; caveats MX/anti-spoof) + **amendement PATCH** de la constitution (table de stack « Formulaire contact » : SMTP M365 → **OVH Email Pro**) avec Sync Impact Report. *(numéro ADR à vérifier vs worktrees parallèles — skill `estuaire-branch-sync`.)*
