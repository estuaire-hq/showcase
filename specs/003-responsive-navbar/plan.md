...
| **VII. Pixel-Perfect Fidelity** | ✅ (build-time) | Skill `estuaire-figma` chargée au build ; 6 frames navbar en cache (`nodes.json`) ; à **lire losslessly** (`figma-node.mjs`) au build + **pull** du node état-actif `51:2699` et de l'asset logo (vecteur). « Pixel-perfect » revendiqué seulement après screenshot-diff par breakpoint (SC-002) ; sinon « non vérifié ». |
...
**Résultat du gate** : PASS. Une seule déviation (II) — consciente, sanctionnée par la spec
(FR-015), tracée en *Complexity Tracking* avec son alternative et son chemin de migration. Aucun
blocage.
