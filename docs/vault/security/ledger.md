...
|---|---|---|---|---|---|---|
| CODE-ORIGIN-EXPOSURE | **atténué** | P1 | **CONFIRMÉ** : `57.128.251.108:443` sert `estuaire.fr` en direct (200, 182 Ko) hors Cloudflare, et accepte un `cf-connecting-ip` forgé | 2026-09-12 | 2026-09-12 | l'origine étant joignable, forger `cf-connecting-ip` donnait un seau neuf à chaque tir et **annulait `CODE-CONTACT-ABUSE`**. Atténué côté application (décision Pierre) par un **quota d'envoi global** de 20 mails / 10 min, clé sur rien de forgeable : vérifié bout en bout, 20 passent, la 21e est en 429, avec 24 IP forgées distinctes. La cause reste entière : l'origine répond toujours hors Cloudflare. **Correctif de fond non fait** (restreindre 80/443 aux plages Cloudflare, ou en-tête secret posé par Cloudflare et vérifié à l'origine), il demande une décision et un accès root sur le VPS |
| CODE-NEXT-AGENTRULES | ouvert | — | `next dev` réécrit un bloc `nextjs-agent-rules` dans `CLAUDE.md` à chaque lancement | 2026-09-12 | | **hors grille**, effet de bord de la montée en 16.3. Sorti du commit de sécurité. À trancher : committer le bloc, ou poser `agentRules: false` dans `next.config.ts` |
